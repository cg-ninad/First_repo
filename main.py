# While putting file as input file renaming not needed
# Description: This script is used to process the data retention alerts and send the alerts to respective teams.
# Version: 1.0
...
class Main:
    ...
    def DataRetention_Process(self):
        """
        The Data Retention process starts from here.
        :return:bool
        """
        ethics_file = ''
        try:
            ...
            if False in ethics_file:
                self.logger.error(f"DataRetention_Process(); File not found: {ethics_file}",)
            ...
            self.logger.info(f"DataRetention_Process(); ECO_List file name found; {ethics_file}")
            ...
            if False in response:
                ...
                filename = response[1]
                if 'retention' not in str(os.path.basename(filename)).lower():
                    msg = "DRT file not found in folder, Invalid file is uploaded. Please upload data retention file " \
                          f"alerts file including <mark>'Data_retention'</mark> keyword in filename.<br> current uploaded file: {filename}"
                    resp = self.mailobj.drt_file_missing(msg)
                    self.mailobj.send_mail(resp[0], resp[1], to=BUSINESS, cc=CC, bcc=BCC)
                    return False
            self.logger.info(f"DataRetention_Process(); Data_retention file name found; {filename}")
            ...
            if len(ip_file) > 1:
                # Mail need to be sent with error to business.
                resp = self.mailobj.drt_file_missing(ip_file[1])
                self.mailobj.send_mail(resp[0], resp[1], to=CC, cc=BUSINESS, bcc=BCC)
                return False
            else:
            ...
            grp_ethics_cc = pd.read_excel(ethics_file, sheet_name='Ethics Team', usecols=["Email Address"])['Email Address'].to_list()
            ...
            #    'Email Address'].to_list()
            india_ids = pd.read_excel(ethics_file, sheet_name='India & Middle East', usecols=["Email Address"])['Email Address'].to_list()

            state = self.process_speakup(ip_file, eco_dict_cc, grp_ethics_cc, ethics_file)
            ...
            self.logger.info("DataRetention_process(); Processing India Region alerts",)

            check_india_file = load_workbook(ip_file)
            check_india_file_list = check_india_file.sheetnames
            if "India & Middle East" in check_india_file_list:
                self.logger.info("DataRetention_process(); India sheet present in the input file")
                print(True)
                ...
                flg = self.india_region_process_data(filename=india_file, ecos=eco_dict_cc, ethics_team=grp_ethics_cc,ethics_file=ethics_file, ind_Non=india_ids)
                if not flg:
                    resp = self.mailobj.file_missing_declare(flg)
                    self.mailobj.send_mail(resp[0], resp[1], to=CC, cc=BUSINESS, bcc=BCC)
                    return False
                ...
                subject, mail_body = self.mailobj.speakup_success()
                self.status_file.close()
                attach = [os.path.basename("Status.txt"), os.path.basename("input_data.xlsx")]  # FIXME:report mail is getting sent here
                status = self.mailobj.send_mail(subject, mail_body, to=BUSINESS, cc=CC, bcc=BCC, attachments=attach)
                if status:
                    resp = self.file_obj.move_files_archive(filename, ARCHIEVE)
                else:
                    return False
            ...
            self.logger.exception(f"DataRetention_process(); Exception Raised: {err}",)
    ...
    def filter_data_ownerwise(self, df):
        ...
        try:
            ...
            df.loc[df['Issue Owner Employee Id'].isin(anne)].to_excel(writer, 'Anne-Violaine',index=False)
            # df.loc[df['Issue Owner Employee Id'].isin(bejoy)].to_excel(writer, 'Bejoy Das', index=False)
            df.loc[~df['Issue Owner Employee Id'].isin(anne)].to_excel(writer, 'input',index=False)
            # writer.save()
            writer.close()
            return [os.path.abspath(ip_file)]
        except Exception as err:
            self.logger.exception(f"filter_data(); failed to filter out data; {err}",)
            return [False, err]
    ...
    def add_region_in_df_basedon_code(self, ip_file=None, ethics_file=None, **krgs):
        ...
        try:
            ...
            # print(region_dict) # this is printing region information from eco list file
            # exit()
            ...
            self.logger.exception(f"add_region_in_df_basedon(); Exception: {excpt}",)
    ...
    def color_formatting(self, wb, ws, filename):
        ...
        :return:updated file name to share with business.
        """
        try:
            ...
            for row in ws.iter_rows(min_row=2):
                try:
                    ...
                    self.logger.error(f"color_formatting(); failed to process row: {e}",)
            wb.save(filename)
            return True
        except Exception as err:
            self.logger.exception(f"Failed to process: {err}",)
            return err.__str__()
    ...
    def excel_font_styles(self, filename):
        ...
        try:
            ...
            self.logger.exception(f"excel_font_styles(); Exception: {err}",)
            return False, err
    ...
    def set_header_color(self, fname):
        ...
        try:
            ...
            self.logger.exception(f"set_header_color(); Exception: {err}",)
            return [False, err]

# NOTE:below method is used to delete Issue owner emp ID column from the file
    def delete_region_column(self, filename):
        ...
        try:
            ...
            self.logger.exception(f"Delete Col Failed: {err}",)
            return [False, err]
    ...
    def process_speakup(self, filename, ecos=None, ethics_team=None, ethics_file=None,**krgs):
        try:
                ...
                try:
                    ...
                    if ws.__contains__('Anne-Violaine'):
                        ...
                        if len(val_df) > 0:
                            ...
                            flg = self.mailobj.send_mail(mail_body[0], mail_body[1], to=issue_owners, cc=ethics_team,bcc=BCC,attachments=['sign.png'])
                            self.status_file.write(str(mail_body[0] + " : " + "Success" + "\n"))
                            if not flg:
                                self.logger.error("DataRetention_process(); Unable to send mail")
                                return False
                    ...
                    #         flg = self.mailobj.send_mail(mail_body[0], mail_body[1], to=issue_owners, cc=ethics_team,
                    #                                      bcc=BCC,
                    #                                      attachments=['sign.png', 'Picture1.png'])
                    #         if not flg:
                    #             self.logger.error("DataRetention_process(); Unable to send mail")
                    #             return False
                    elif ws.__contains__('India & Middle East'):
                        pass
                        ...
                        if len(val_df) > 0:
                            ...
                            # df.to_html('table.html') #TODO:check this thoroughly once get sometime
                            xlsx2html(filename, sheet=ws, output='table.html')
                            body_table = open(os.path.abspath('table.html')).read()
                            mail_body = self.mailobj.speakup_mail(body_table, ws)
                            flg = 1
                            # flg = self.mailobj.send_mail(mail_body[0], mail_body[1], to=issue_owners, cc=ecos_list,attachments=['sign.png', 'Picture1.png'])
                            # TODO: mail is getting sent here
                            flg = self.mailobj.send_mail(mail_body[0], mail_body[1], to=issue_owners, cc=ecos_list,bcc=BCC,attachments=['sign.png'])
                            self.status_file.write(str(mail_body[0] + " : " + "Success" + "\n"))
                            if not flg:
                                self.logger.error("DataRetention_process(); Unable to send mail")
                                return False
                    ...
                    self.logger.exception(f"DataRetention_process(); Exception while processing alerts; {err}")
                    continue
        except Exception as err:
            self.logger.exception(f"DataRetention_process(); Exception while processing alerts; {err}")
            return [False, err]
    ...
    def _delete_emp_id_(self, filename, ws, **krgs):
        try:
            ...
            # ws1.delete_cols(6)
            ws1.delete_cols(4)
            print(ws1.sheet_view)
            # ws1.delete_cols(10)
            book.save(filename)
        except Exception as err:
            self.logger.exception(f"_delete_emp_id_(); Exception while deleting; {err}",)
    ...
    def __split_data_by_team__(self, filename):
        ...
        try:
            ...
            df = pd.read_excel(filename, sheet_name="input")
            df['Region'] = df['Region'].fillna('India & Middle East')
            dict_of_country = dict(iter(df.groupby('Region')))
            writer = pd.ExcelWriter(countrywise_file, engine='openpyxl')
            for key, val in dict_of_country.items():
                val.to_excel(writer, sheet_name=key, header=True, index=False)
    ...
    def add_india_teams_to_file(self, filename, ethics_file):
        ...
        try:
            ethics_df = pd.read_excel(ethics_file, sheet_name='India & Middle East', usecols=["Region", "Employee ID"])
            region_dict = dict(ethics_df.values)
            ethics_df.replace(r"^ +| +$", r"", regex=True, inplace=True)
            india_df = pd.read_excel(filename, sheet_name='India & Middle East')
            india_df.replace(r"^ +| +$", r"", regex=True, inplace=True)
            ...
            self.logger.exception(f"add_india_teams_to_file(); Exception: {err}",)
            return [False, err]

    def india_region_process_data(self, filename, ecos=None, ethics_team=None, ind_ethics=None, ind_hr=None,ind_Non=None, ethics_file=None, **krgs):
        try:
            # cc =ethics_team
            ...
            for iws in india_ws:
                ...
                body_table = open(os.path.abspath('table.html')).read()
                mail_body = self.mailobj.speakup_mail(body_table, "India & Middle East")
                ...
                if iws.__contains__('India Ethics'):
                    ...
                    if len(val_df) > 0:
                        ...
                        flg = self.mailobj.send_mail(mail_body[0], mail_body[1], to=ind_owners, cc=ind_ethics,bcc=BCC,attachments=['sign.png'])
                        self.status_file.write(str(mail_body[0] + " : " + "Success" + "\n"))
                        if not flg:
                            self.logger.error("DataRetention_process(); Unable to send mail")
                            return False
                if iws.__contains__('India & Middle East'):
                    self.logger.info("***************INDIA & Middle East**************")
                    val_df = pd.read_excel(filename, sheet_name=iws)
                    if len(val_df) > 0:
                        self.logger.info(f"india_region_process_data(); {iws}")
                        ind_ids = list(np.setdiff1d(ind_Non, ind_owners))
                        ind_Non.extend(ethics_team)
                        ind_Non = list(set(ind_Non))

                        ind_Non = list(np.setdiff1d(ind_Non, ind_owners))

                        flg = self.mailobj.send_mail(mail_body[0], mail_body[1], to=ind_owners, cc=ind_Non,bcc=BCC,attachments=['sign.png'])
                        self.status_file.write(str(mail_body[0] + " : " + "Success" + "\n"))
                        if not flg:
                            self.logger.error("DataRetention_process(); Unable to send mail")
                            return False
                    ...
                    if len(val_df) > 0:
                        ...
                        flg = self.mailobj.send_mail(mail_body[0], mail_body[1], to=ind_owners, cc=ind_hr,bcc=BCC,attachments=['sign.png'])
                        self.status_file.write(str(mail_body[0] + " : " + "Success" + "\n"))
                        if not flg:
                            self.logger.error("DataRetention_process(); Unable to send mail")
                            return False
                    ...
                    val_df = pd.read_excel(filename, sheet_name=iws)
                    if len(val_df) > 0:
                        self.logger.info(f"ind_owners: {ind_owners} \n ethics_team: {ethics_team} \n")
                        self.logger.info(f"india_region_process_data(); {iws}")
                        ind_hr_ids = list(np.setdiff1d(ind_Non, ind_owners))
            ...
            self.logger.exception(f"__india_region_process_data(); Exception: {err}",)
            return [False, err]

    def main(self):

        try:
            state = self.DataRetention_Process()
            if state:
            ...
            self.logger.exception(f"-----END----- Error: {err}")
            exit(1)
