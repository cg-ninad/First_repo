...
class Main:
    ...
    def speakup_process(self):
        ...
        try:
            ...
            if not state:
                resp = self.mailobj.drt_file_missing(state)
                self.mailobj.send_mail(resp[0], resp[1], to=CC, cc=BUSINESS, bcc=BCC)
                return False
            ...
            if "India" in check_india_file_list:
                ...
                flg = self.delete_region_column(india_file)
                if not flg:
                    resp = self.mailobj.file_missing_declare(flg)
                    self.mailobj.send_mail(resp[0], resp[1], to=CC, cc=BUSINESS, bcc=BCC)
                    return False
                ...
                self.logger.info("speakup_process(); India is not present in the input file", extra=self.extra_dict_common)
                subject, mail_body = self.mailobj.speakup_success()
                self.status_file.close()           
    ...
    def filter_data_ownerwise(self, df):
        ...
        try:
            ...
            df.loc[df['Issue Owner'].isin(['Anne-Violaine AGAZZI-MONNIE'])].to_excel(writer, 'Anne-Violaine',
                                                                                     index=False)
            df.loc[df['Issue Owner'].isin(['Bejoy Das'])].to_excel(writer, 'Bejoy Das', index=False)
            df.loc[~df['Issue Owner'].isin(['Anne-Violaine AGAZZI-MONNIE', 'Bejoy Das'])].to_excel(writer, 'input', index=False)
            writer.save()
        ...
        try:
            ...
            for row in ws.iter_rows(min_row=2):
                try:
                    ...
                    if (int(delta.days) > 7) and ('question' in intake_type.lower()):
                        alert_cell.font = red_ft
